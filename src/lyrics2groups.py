...
parser.add_argument(
    "input_text", help="input directory path with .txt files of lyrics/literature"
)
parser.add_argument(
    "phones_path", help=".phones file path with format the same as cmudict.phones"
)
parser.add_argument("model_dir_path", help="g2p model directory path")
parser.add_argument(
    "-o", "--output_dir_path", help="output directory path with for <name>.html file"
)
...
if args.evaluate:
    assert args.input_text == "src/eval_group/figaro.txt", (
        "Evaluation only" + "works with src/eval_group/figaro.txt as the input text"
    )

if args.phoneme_list == phoneme_list_default and not os.path.exists(
    "./" + phoneme_list_default
):
    os.mkdir("./" + phoneme_list_default)

if args.unknown_list_dir == unknown_list_default and not os.path.exists(
    "./" + unknown_list_default
):
...
word_list = utils.text_to_word_list(args.input_text, args.unknown_list_dir, filler)
# print("Word list is " + str(word_list))
utils.words_to_phonemes(args.unknown_list_dir, args.phoneme_list, args.model_dir_path)


def get_unknown_phoneme_path(directory):
    if len(os.listdir(directory)) > 1:
        raise Exception(
            "Expected only 1 unknown list in "
            + directory
...
groups = clustering.cluster(
    syllable_lines, ignore_set, verse_tracking=False, num_iterations=args.num_iterations
)
